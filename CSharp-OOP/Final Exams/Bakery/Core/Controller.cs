...
namespace Bakery.Core
{
    public class Controller : IController
    {
        ...
        {
            string result = "";
            List<ITable> tablesIsNotReserved = tables.Where(x => !x.IsReserved).ToList();

            foreach (var table in tablesIsNotReserved)
            {
                result += table.GetFreeTableInfo() + Environment.NewLine;
            }
           // for (int i = 0; i < tablesIsNotReserved.Count; i++)
          //  {
               // if(result == tablesIsNotReserved.Count - 1)
           //     result += tablesIsNotReserved[i].GetFreeTableInfo() + Environment.NewLine;
           // }
        ...
        public string OrderDrink(int tableNumber, string drinkName, string drinkBrand)
        {
            ...
            {
                var drink = drinks.FirstOrDefault(drink => drink.Name == drinkName);
